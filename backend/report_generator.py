...
def generate_governance_analysis(top_features):
    ...
    # SAFETY FIX
    if gov_data is None or gov_data.empty:
    ...
    for _, row in gov_data.iterrows():
        ...
        impact = (
            ...
            if row['shap_value'] > 0
            else "acting as a stabilizing buffer"
        )

        narrative += (
            "- "
            + str(row['feature']).replace('_', ' ')
            + " is "
            + impact
            + ".\n"
        )
...
def generate_conflict_analysis(top_features):
    ...
    # SAFETY FIX
    if con_data is None or con_data.empty:
    ...
    for _, row in con_data.iterrows():
        ...
        impact = (
            "elevating escalation risk"
            if row['shap_value'] > 0
            else "suggesting de-escalation potential"
        )
...
def generate_economic_analysis(top_features):
    ...
    # SAFETY FIX
    if eco_data is None or eco_data.empty:
    ...
    for _, row in eco_data.iterrows():
        ...
        impact = (
            "exacerbating fragility"
            if row['shap_value'] > 0
            else "providing economic resilience"
        )
...
def generate_full_report(country, year, probability, risk_level, top_features):
    ...
    report = {
        ...
        "executive_summary": generate_risk_summary(
            country,
            year,
            probability,
            risk_level
        ...
        "governance_analysis": generate_governance_analysis(
            top_features
        ),

        "conflict_analysis": generate_conflict_analysis(
            top_features
        ),
